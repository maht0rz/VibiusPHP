...
class Storage
{
    public static function sort($data,$type = 'asc'){
        foreach ($data as $key => $row) {
                $c[$key] = $row['created'];
            }
            if($type == 'asc' || $type == 'ASC'){
              array_multisort($c, SORT_ASC, $data);  
             }else if($type = 'desc' || $type == "DESC"){
    ...
    public function update($data)
    {
        $file = dirname(
                __FILE__
            ) . "/../../App/Storage/" . $this->database . "/" . $this->table . "/" . $this->row . ".php";
        if (file_exists($file)) {
            $struct = dirname(
                    __FILE__
                ) . "/../../App/Storage/" . $this->database . "/" . $this->table . "/__structure.php";
